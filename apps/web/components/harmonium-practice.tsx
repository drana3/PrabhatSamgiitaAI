      ...
      {notation ? (
        <div className="border-t border-gold-500/20 p-5 sm:p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-navy-900/10 bg-gold-50 p-3">
            ...
            <div className="flex flex-wrap rounded-lg border border-navy-900/10 bg-white p-1"><ModeButton active={system === "sargam"} onClick={() => setSystem("sargam")}>Sargam</ModeButton><ModeButton active={system === "keys"} onClick={() => setSystem("keys")}>Keyboard keys</ModeButton><ModeButton active={system === "guide"} onClick={() => setSystem("guide")}>Sargam guide</ModeButton></div>
...
function SargamGuide({ tonic }: { tonic: string }) {
  ...
  return <section className="mt-4 rounded-2xl bg-navy-950 p-5 text-white sm:p-6">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold-300">The seven swaras</p><h3 className="mt-2 font-serif text-2xl">Sargam at a glance</h3></div><p className="text-xs text-navy-100">Current Sa: <strong className="text-white">{tonic}</strong></p></div>
