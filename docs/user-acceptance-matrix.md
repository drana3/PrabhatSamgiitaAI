...
| Meaning is unavailable | A truthful preparation state appears; no invented meaning or blank card. |
| Audio exists | Play control appears without displaying its source URL. |
| Audio is absent | A deliberate availability message appears. |
...
| Video is absent | Channel-watcher status appears instead of an empty panel. |
