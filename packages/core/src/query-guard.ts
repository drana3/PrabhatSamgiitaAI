...
export const songRangeGuidance =
...
export function queryIsUseful(value: string, maxLength = 600, options: QueryGuardOptions = {}) {
  ...
  if (!companion && !hasSongContext(normalized) && (numericParts.length > 1 || numericParts.some((part) => part.length > 4))) {
    return false
  }
...
export function queryGuidanceFor(value: string, options: QueryGuardOptions = {}) {
  ...
  if (explicitSongNumber && (Number(explicitSongNumber[1]) < 1 || Number(explicitSongNumber[1]) > 5018)) {
    return songRangeGuidance
  }
  if (/^\d{1,4}$/.test(normalized) && (Number(normalized) < 1 || Number(normalized) > 5018)) {
    return songRangeGuidance
  }
  if (options.companion && !options.allowFollowUp && !hasSongContext(normalized) && !indicScript.test(normalized)) {
    const words = normalized.toLocaleLowerCase().match(/[^\W\d_]+/gu) ?? []
    if (words.length <= 2 && (words.length <= 1 || words.every((word) => vagueFillers.has(word)))) {
      return unrelatedGuidance
    }
  }
